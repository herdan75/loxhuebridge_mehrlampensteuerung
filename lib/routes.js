...
function persistSettings(method, candidate, res) {
    try {
        configManager[method](candidate);
        return true;
    } catch (error) {
        logger.error('Speichern fehlgeschlagen: ' + error.message, 'SYSTEM');
        res.status(500).json({ success: false, error: 'Speichern fehlgeschlagen. Bisherige Einstellungen bleiben erhalten.' });
        return false;
    }
}
...
router.post('/api/setup/register', async (req, res) => { 
    try { 
        const r = await axios.post(`https://${req.body.ip}/api`, { devicetype: "loxHueBridge" }, { httpsAgent: hueManager.httpsAgent, timeout: 5000, maxRedirects: 0 });
        if(r.data[0].success) { 
            configManager.saveConfig({ ...configManager.config, bridgeIp: req.body.ip, appKey: r.data[0].success.username });
...
router.post('/api/setup/loxone', (req, res) => { 
    const c = structuredClone(configManager.config);
    ...
    if (!persistSettings('saveConfig', c, res)) return;
    ...
    try {
        ...
        if (resources.light) {
            resources.light.forEach(x => {
                const device = hueManager.getServiceToDeviceMap()[x.id];
                t.push({ uuid: x.id, name: x.metadata?.name || x.id, type: 'light', deviceId: device?.deviceId || x.owner?.rid, deviceName: device?.deviceName, capabilities: lightCapabilities[x.id] || null });
            });
        }
...
router.post('/api/mapping', (req, res) => {
    if (!Array.isArray(req.body) || req.body.some(m => !m || typeof m.loxone_name !== 'string')) {
        return res.status(400).json({ success: false, error: 'Ungültiges Mapping' });
    }
    if (!persistSettings('saveMapping', req.body.filter(m => m.loxone_name), res)) return;
...
router.post('/api/mapping/:loxoneName/settings', (req, res) => {
    ...
    try {
        ...
        const updated = { ...entry, ...settings };
        if (!persistSettings('saveMapping', configManager.mapping.map(item => item === entry ? updated : item), res)) return;
...
router.post('/api/security/settings', async (req, res) => {
    ...
    if (user.includes(':') || password.length > 1024) {
        return res.status(400).json({ success: false, error: 'Ungültiger Benutzername oder Passwort zu lang.' });
    }
    let passwordHash;
    ...
    const candidate = { ...configManager.config };
    if (passwordHash) candidate.authPasswordHash = passwordHash;
    if (enabled && !candidate.authPasswordHash) {
        return res.status(400).json({ success: false, error: 'Bitte zuerst ein Passwort setzen.' });
    }

    candidate.authEnabled = enabled;
    candidate.authUser = user;
    if (!persistSettings('saveConfig', candidate, res)) return;
...
router.post('/api/settings/debug', (req, res) => { 
    if (!persistSettings('saveConfig', { ...configManager.config, debug: !!req.body.active }, res)) return;
...
router.post('/api/system/restart', (req, res) => { 
    ...
    setTimeout(() => lifecycle.emit('shutdown', 'API-Neustart'), 500);
});
...
router.post('/api/system/restore', (req, res) => {
    try {
        ...
        configManager.restore({ ...configManager.config, ...backup.config }, backup.mapping);
        logger.success("Restore erfolgreich!", "SYSTEM");
        res.json({success: true});
        setTimeout(() => lifecycle.emit('shutdown', 'Backup-Restore'), 1000);
    } catch(e) {
...
router.get('/:name/:value', async (req, res) => {
    ...
    if (isGlobalAll || isMappedAll) {
        ...
        try {
            const homeRes = await axios.get(`https://${configManager.config.bridgeIp}/clip/v2/resource/bridge_home`, hueManager.getHueRequestConfig());
