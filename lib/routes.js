...
router.post('/api/setup/loxone', (req, res) => { 
    ...
    if(req.body.throttleTime!==undefined) {
        ...
        hueManager.REQUEST_QUEUES.grouped_light.delayMs = Math.max(c.throttleTime, 1000);
    }
    ...
    if(req.body.multiLightControl !== undefined) {
        c.multiLightControl = {
            ...(c.multiLightControl || {}),
            syncWindowMs: parseNumberSetting(req.body.multiLightControl.syncWindowMs, 120, 10, 2000),
            batchSize: parseNumberSetting(req.body.multiLightControl.batchSize, 4, 1, 50),
            batchDelayMs: parseNumberSetting(req.body.multiLightControl.batchDelayMs, 30, 0, 2000),
            maxCommandsPerSecond: parseNumberSetting(req.body.multiLightControl.maxCommandsPerSecond, 10, 1, 50)
        };
    }
...
router.get('/api/settings', (req, res) => res.json({ 
    ...
    multiLightControl: configManager.config.multiLightControl || { syncWindowMs: 120, batchSize: 4, batchDelayMs: 30, maxCommandsPerSecond: 10 }
}));
...
router.get('/:name/:value', async (req, res) => {
    ...
    if (isGlobalAll || isMappedAll) {
        ...
        (async () => {
            ...
            const multiSync = hueManager.getMultiSyncPreview([]);
            const lightDelay = Math.ceil(1000 / multiSync.settings.maxCommandsPerSecond);
            const groupDelay = hueManager.REQUEST_QUEUES.grouped_light.delayMs;
            for (const target of targets) {
                hueManager.executeCommand(target, value, 0);
                const delay = target.hue_type === 'group' ? groupDelay : lightDelay;
