...
let multiSyncGenerations = new Map();
...
function applyRuntimeConfig() {
    ...
    REQUEST_QUEUES.light.delayMs = throttleTime;
    REQUEST_QUEUES.grouped_light.delayMs = Math.max(throttleTime, 1000);
}
...
function getHueSchedulerSpacingMs(resourceType = 'light') {
    const globalSpacingMs = Math.ceil(1000 / getBridgeMaxCommandsPerSecond());
    if (resourceType === 'grouped_light') {
        return Math.max(globalSpacingMs, REQUEST_QUEUES.grouped_light.delayMs);
    }
...
async function processHueScheduler() {
    ...
    while (hueSchedulerQueue.length > 0) {
        ...
        const now = Date.now();
        const waitMs = Math.max(0, nextHueSchedulerSlotAt - now);
        ...
        try {
            ...
            nextHueSchedulerSlotAt = Date.now() + getHueSchedulerSpacingMs(task.resourceType);
        }
    }
...
function getMultiSyncGeneration(groupId) {
    return multiSyncGenerations.get(normalizeMultiSyncGroupId(groupId)) || 0;
}

function startNewMultiSyncGeneration(groupId) {
    const normalizedGroupId = normalizeMultiSyncGroupId(groupId);
    const generation = getMultiSyncGeneration(normalizedGroupId) + 1;
    multiSyncGenerations.set(normalizedGroupId, generation);
    return generation;
}
...
function scheduleMultiSyncedLightCommand(entry, uuid, type, payload, forcedTransition = null) {
    ...
    const generation = startNewMultiSyncGeneration(groupId);
    ...
    const timer = setTimeout(() => {
        ...
        buildMultiSyncSchedule(batch, settings).forEach(({ item, delayMs }) => {
            setTimeout(() => {
                if (getMultiSyncGeneration(groupId) !== generation) {
                    logger.debug(`Multi-Sync ${groupId.toUpperCase()}: verwerfe alten Timer für ${item.entry.loxone_name}`, 'LIGHT');
                    return;
                }

                {
                    if (getMultiSyncGeneration(groupId) !== generation) {
                        logger.debug(`Multi-Sync ${groupId.toUpperCase()}: verwerfe alten Bridge-Slot für ${item.entry.loxone_name}`, 'LIGHT');
                        return;
                    }
...
async function executeCommand(entry, value, forcedTransition = null) {
    ...
    if (n === 0) payload = { on: { on: false } };
        ...
        if (s.startsWith('20') && s.length >= 9) {
            ...
            assertLoxonePercentComponent(b, 'Brightness', s);
...
module.exports = {
    ...
    _internals: {
        ...
        getMultiSyncGeneration,
        startNewMultiSyncGeneration,
        mergeHuePayload,
        isOffPayload,
