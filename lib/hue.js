...
function getMultiSyncSettings() {
    ...
    return {
        ...
        maxCommandsPerSecond: Number.isFinite(Number(cfg.maxCommandsPerSecond))
            ? Math.max(1, Math.min(50, Number(cfg.maxCommandsPerSecond)))
            : MULTI_SYNC_DEFAULTS.maxCommandsPerSecond
    };
}
...
function getMultiSyncPreview(entries = configManager.mapping) {
    const items = entries
        .filter(entry => entry.hue_type === 'light' && entry.multi_sync === true)
        .map(entry => ({ entry }));
...
function scheduleMultiSyncedLightCommand(entry, uuid, type, payload, forcedTransition = null) {
    ...
    multiSyncTimer = setTimeout(() => {
        const batch = Array.from(multiSyncBuffer.values());
        multiSyncBuffer = new Map();
        multiSyncTimer = null;
