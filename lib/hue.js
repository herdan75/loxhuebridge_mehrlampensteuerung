...
async function putHueWithRateLimitRetry(url, payload, options = {}) {
    ...
    const { maxRetries: _maxRetries, retryDelaysMs: _retryDelaysMs, ...requestOptions } = options;

    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            return await axios.put(url, payload, getHueRequestConfig(requestOptions));
        } catch (error) {
...
function scheduleStateVerification(uuid, type, payload, loxName, commandGeneration, options = {}) {
    ...
    verifyDelaysMs.forEach((delayMs, index) => {
        ...
        const timer = setTimeout(() => {
            if (getLampCommandGeneration(uuid) !== commandGeneration) return;
...
async function putHuePayload(uuid, type, payload, loxName, options = {}) {
    ...
    return scheduleHuePutTask({
        ...
        execute: async () => {
            ...
            if (type === 'light') recordLampCommand(uuid, loxName, payload, commandGeneration, isFollowUp);

            const response = await putHueWithRateLimitRetry(url, payload);
            if (type === 'light') recordHueCommunicationErrors(uuid, loxName, response);

            if (payload.on !== undefined) updateStatus(loxName, 'on', payload.on.on ? 1 : 0);
...
async function putLightEffectPayload(uuid, payload, loxName, options = {}) {
    ...
    return scheduleHuePutTask({
        ...
        execute: async () => {
            const response = await putHueWithRateLimitRetry(url, payload);
            recordHueCommunicationErrors(uuid, loxName, response);
...
async function updateLightWithQueue(uuid, type, payload, loxName, forcedDuration = null, options = {}) {
    ...
    if (shouldSplitOnCommand(entry, type, payload, preparedOptions)) {
        ...
        const timer = setTimeout(() => {
            if (getLampCommandGeneration(uuid) !== preparedOptions.commandGeneration) return;
...
async function executeAlert(entry) {
    ...
    try {
        ...
        await scheduleHuePutTask({
            ...
            execute: async () => {
                const response = await putHueWithRateLimitRetry(url, { alert: { action: 'breathe' } });
                if (rtype === 'light') recordHueCommunicationErrors(rid, entry.loxone_name, response);
            }
        });
    } catch (e) { logger.hueError(e, 'LIGHT'); }
}
