...
function buildMultiSyncSchedule(items, settings = getMultiSyncSettings()) {
    ...
    return items
        ...
        .map((scheduleItem, sortedIndex) => {
            const batchDelay = Math.floor(sortedIndex / batchSize) * batchDelayMs;
            const requestedDelay = baseDelayMs + scheduleItem.offset + (sortedIndex * commandSpacingMs) + batchDelay;

            return { ...scheduleItem, requestedDelay: Math.max(0, Math.round(requestedDelay)) };
        })
        .map(scheduleItem => {
