...
function getMultiSyncSettings() {
    const cfg = configManager.config.multiLightControl || {};

    return {
        syncWindowMs: Number.isFinite(Number(cfg.syncWindowMs))
            ? Number(cfg.syncWindowMs)
            : MULTI_SYNC_DEFAULTS.syncWindowMs,
        batchSize: Number.isFinite(Number(cfg.batchSize))
            ? Math.max(1, Number(cfg.batchSize))
            : MULTI_SYNC_DEFAULTS.batchSize,
        batchDelayMs: Number.isFinite(Number(cfg.batchDelayMs))
            ? Math.max(0, Number(cfg.batchDelayMs))
            : MULTI_SYNC_DEFAULTS.batchDelayMs
    };
}
...
async function putHuePayload(uuid, type, payload, loxName) {
    ...
    await axios.put(url, payload, {
        headers: { 'hue-application-key': configManager.config.appKey },
        httpsAgent
    });
...
function scheduleMultiSyncedLightCommand(entry, uuid, type, payload, forcedTransition = null) {
    ...
    if (multiSyncTimer) return;

    const settings = getMultiSyncSettings();

    multiSyncTimer = setTimeout(() => {
        const batch = multiSyncBuffer;
        multiSyncBuffer = [];
        multiSyncTimer = null;

        const baseTime = Date.now() + settings.syncWindowMs;

        batch.forEach((item, index) => {
            const offset = getSyncOffsetMs(item.entry);
            const batchDelay = Math.floor(index / settings.batchSize) * settings.batchDelayMs;
            const delay = Math.max(0, baseTime + offset + batchDelay - Date.now());
...
async function executeAlert(entry) {
    ...
    try {
        ...
        await axios.put(url, { alert: { action: 'breathe' } }, { headers: { 'hue-application-key': configManager.config.appKey }, httpsAgent });
    } catch (e) { logger.hueError(e, 'LIGHT'); }
}

async function executeEffect(entry, effectName) {
    const rid = entry.hue_uuid;
    const url = `https://${configManager.config.bridgeIp}/clip/v2/resource/light/${rid}`;
    try {
        logger.info(`EFFECT ${effectName} -> ${entry.loxone_name}`, 'LIGHT');
        await axios.put(url, { effects: { effect: effectName } }, { headers: { 'hue-application-key': configManager.config.appKey }, httpsAgent });
    } catch (e) { logger.hueError(e, 'LIGHT'); }
}
...
async function executeTimedEffect(entry, effectName, durationSeconds) {
    ...
    try {
        logger.info(`TIMED EFFECT ${effectName} ${durationSeconds}s -> ${entry.loxone_name}`, 'LIGHT');
        await axios.put(url, { timed_effects: { effect: effectName, duration: durationMs } }, { headers: { 'hue-application-key': configManager.config.appKey }, httpsAgent });
    } catch (e) { logger.hueError(e, 'LIGHT'); }
}
...
async function startEventStream() {
    ...
    try {
        ...
        response.data.on('data', (chunk) => {
            ...
            const rawEvents = eventBuffer.split(/\r?\n\r?\n/);
        ...
        response.data.on('end', () => { logger.warn("EventStream vom Server beendet.", 'SYSTEM'); eventStreamActive = false; setTimeout(startEventStream, 5000); });
        response.data.on('error', (err) => { logger.error("EventStream Fehler: " + err.message, 'SYSTEM'); eventStreamActive = false; setTimeout(startEventStream, 5000); });
    } catch (error) { logger.error("EventStream Verbindungsfehler: " + error.message, 'SYSTEM'); eventStreamActive = false; setTimeout(startEventStream, 10000); }
}
