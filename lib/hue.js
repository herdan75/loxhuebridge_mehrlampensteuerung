...
async function sendToHueRecursive(uuid, type, payload, loxName) {
    enqueueRequest(type, async () => {
        try {
            ...
            await axios.put(url, payload, { headers: { 'hue-application-key': configManager.config.appKey }, httpsAgent });
            updateStatus(loxName, 'on', payload.on?.on ? 1 : 0);
            if(payload.dimming) updateStatus(loxName, 'bri', payload.dimming.brightness);
        } catch (e) { logger.hueError(e, 'LIGHT'); } finally {
...
async function startEventStream() {
    ...
    try {
        ...
        response.data.on('data', (chunk) => { lastEventTimestamp = Date.now(); const lines = chunk.toString().split('\n'); lines.forEach(line => { if (line.startsWith('data: ')) { try { processHueEvents(JSON.parse(line.substring(6))); } catch (e) { logger.error("EventStream JSON Parsing Fehler: " + e.message, "SYSTEM"); } } }); });
